...
//     `node scripts/ci-matrix.mts --emit >> "$GITHUB_OUTPUT"` and the matrix
//     jobs feed those outputs into strategy.matrix via fromJSON.
//   - scripts/test-ci.mts — the local CI runner imports these exports to
...
export const CLIENT_PINS: ClientPin[] = [
...
// Only runs on direct execution, never when test-ci.mts imports this module.
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  if (process.argv[2] !== "--emit") {
    console.error("usage: pnpm tsx scripts/ci-matrix.mts --emit");
    process.exit(2);
  }
