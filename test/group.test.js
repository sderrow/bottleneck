...
describe("Group", function () {
  ...
  it("Should pass new limiter to 'created' event", function () {
    ...
    group.on("created", function (created, key) {
      keys.push(key);
      promises.push(
        created.updateSettings({ id: key }).then(function (limiter) {
          ids.push(limiter.id);
        }),
      );
    });
    ...
    return Promise.all([
      ...
      .then(function () {
        ...
        limiters.forEach(function (limiter, i) {
          ...
          assert(
            limiter.limiter instanceof Bottleneck,
            "Expected limiter.limiter to be a Bottleneck",
          );
        });
  ...
  it("Should call autocleanup", function () {
    ...
    return group
      ...
      .then(function () {
        c.mustExist(group.instances[KEY]);
        return new Promise(function (resolve, _reject) {
          setTimeout(resolve, 100);
        });
      })
      .then(function () {
        c.mustNotExist(group.instances[KEY]);
      });
  });
});
