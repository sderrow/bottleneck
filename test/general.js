...
describe("General", function () {
  ...
  describe("Events", function () {
    ...
    it("Should fire events on empty queue", function () {
      ...
      return c
        .pNoErrVal(c.limiter.schedule({ id: 1 }, c.slowPromise, 50, null, 1), 1)
        .then(function () {
    ...
    it("Should fire events once", function () {
      ...
      c.limiter.once("empty", function () {
        calledEmptyOnce++;
      });
      c.limiter.once("idle", function () {
        calledIdleOnce++;
      });
      c.limiter.on("empty", function () {
        calledEmpty++;
      ...
      return c
        .pNoErrVal(c.limiter.schedule(c.promise, null, 2), 2)
        .then(function () {
    ...
    it("Should wait for async event listeners", function (done) {
      ...
      c.limiter.on("empty", function () {
        return c.slowPromise(100, null, 1, 2).then(function (x) {
          c.mustEqual(x, [1, 2]);
  ...
  describe("Weight", function () {
    it("Should not add jobs with a weight above the maxConcurrent", function () {
      ...
      return c.limiter
        .schedule({ weight: 3 }, c.promise, null, 3)
        .catch(function (err) {
          c.mustEqual(
            err.message,
    ...
    it("Should support custom job weights", function () {
      ...
      return c.last().then(function (_results) {
        c.checkDuration(400);
    ...
    it("Should overflow at the correct rate", function () {
      ...
      var p1 = c.pNoErrVal(
        c.limiter.schedule({ weight: 1, id: 1 }, c.slowPromise, 100, null, 1),
        1,
      );
      var p2 = c.pNoErrVal(
        c.limiter.schedule({ weight: 2, id: 2 }, c.slowPromise, 150, null, 2),
        2,
      );
      var p3 = c.pNoErrVal(
        c.limiter.schedule({ weight: 1, id: 3 }, c.slowPromise, 100, null, 3),
        3,
      );
      var p4 = c.pNoErrVal(
        c.limiter.schedule({ weight: 1, id: 4 }, c.slowPromise, 100, null, 4),
        4,
      );
  ...
  describe("Expiration", function () {
    it("Should cancel jobs", function () {
      ...
      return Promise.all([
        c.pNoErrVal(
          c.limiter.schedule({ id: "very-slow-no-expiration" }, c.slowPromise, 150, null, 1),
          1,
        ),

        c.limiter
          .schedule({ expiration: 50, id: "slow-with-expiration" }, c.slowPromise, 75, null, 2)
          .then(function () {
          ...
          .catch(function (err) {
            ...
            c.mustGt(duration, 45);
            c.mustLt(duration, 80);
        ...
        .then(function () {
          var duration = Date.now() - t0;
          c.mustGt(duration, 145);
          c.mustLt(duration, 180);
          return Promise.all([c.limiter.running(), c.limiter.done()]);
        })
        .then(function ([running, done]) {
          c.mustEqual(running, 0);
  ...
  describe("Reservoir Increase", function () {
    it("Should auto-increase the reservoir", async function () {
      ...
      await c.last({ weight: 0, priority: 9 });
      c.checkResultsOrder([[1], [2], [3], [4], [5]]);
      c.mustEqual(calledDepleted, 1);
      c.checkDuration(450);
    });
