...
describe("Promises", function () {
  ...
  it("Should pass error on failure", function () {
    ...
    return c.limiter.schedule(c.promise, new Error(failureMessage)).catch(function (err) {
  ...
  it("Should get rejected when rejectOnDrop is true", function () {
    ...
    return c.limiter
      .schedule({ id: 3 }, c.slowPromise, 50, null, 3)
      .catch(function (err) {
  ...
  describe("Wrap", function () {
    ...
    it("Should inherit the original target for wrapped methods", function () {
      ...
      var object = {
        fn: c.limiter.wrap(function () {
          return this;
        }),
      };

      return object.fn().then((result) => {
        c.mustEqual(result, object);
      });
    });
    ...
    it("Should allow passing options", function () {
      ...
      return wrapped
        .withOptions({ priority: 1 }, new Error(failureMessage), 6)
        .catch(function (err) {
          c.mustEqual(err.message, failureMessage);
          return c.last();
        })
